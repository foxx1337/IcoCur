    ...
    public void AddImageToList(uint width, uint height, uint bpp, uint compression)
    {
        if (compression != 0U)
        {
            this.listBox1.Items.Add(
                (object)(Convert.ToString(width) + "x" + Convert.ToString(height) + ", (compressed)"));
        }
        else
        {
            this.listBox1.Items.Add((object)(Convert.ToString(width) + "x" + Convert.ToString(height) + ", " +
                                             Convert.ToString(bpp) + "-bit"));
        }
    }
